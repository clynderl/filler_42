...
/*   Updated: 2020/10/19 17:08:41 by clynderl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		parse_board_line(char *line, int *board_line, int width, int player)
{
	...
	while (i < width)
	{
		if (line[i] == '.')
			board_line[i] = 0;
		else if (ft_toupper(line[i]) == parse_player_char(player))
			board_line[i] = -1;
		else if (ft_toupper(line[i]) == parse_enemy_char(player))
			board_line[i] = -2;
		else
			return (0);
		i++;
...
int		parse_board(t_board *board, int player)
{
	...
	{
		if (!get_next_line(0, &line)
			|| !parse_board_line(&line[4], board->heatmap[j], board->width, player))
		{
			ft_strdel(&line);
			return (0);
		}
